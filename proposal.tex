...
misconceptions. This piece of data also helps me confirm that
anticipated wrong answers represent the expected misconceptions.
...
errors. I plan to adjust the process described in
\Cref{s:find-misconceptions-with-tutor} so that it presents programs
...
data from all populations. I plan to explore more literature and
...
The tutor is mostly consisted of \emph{interpreting tasks}:
...
obviously wrong. The tutor is adding options with an ad hoc rule.
Eventually, I plan to generate options by moving and/or copying
numbers in the current options. In this case, my plan would add
\lstinline|12 12| and \lstinline|0 12|. In summary, each MCQ has the
following options:
\begin{itemize}
...
\paragraph{Penalty to Wrong Choices} To discourage students from
making a random choice, the tutor has been giving penalty. Designing
the strategy of assigning penalty is non-trivial:
\begin{itemize}
...
\end{quote}
If a student gave a wrong answer that does not correspond to a
...
The prompt should ask about the behavior of new language constructs
...
how programs runs with notional machines (NMs) is beneficial for
...
of the wrong answers correspond to more than one misconception; some
misconceptions are represented by only one question; the order of
interpreting tasks are not randomized. I plan to tackle the first two
...
\begin{enumerate}
  \item Ask students to explain when they gave a wrong answer in an
        interpreting question. With student explanations, I check whether
...
expect these datasets and the datasets from Spring 2024 would give a
