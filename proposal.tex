...
\newcommand{\rqI}{What are the misconceptions that are about program behavior, apply to most modern programming languages, and present (even) in students with prior programming backgrounds?}
...
\label{s:methodology-rq1}

\begin{description}
  \item[RQ1] \rqI
\end{description}

To ensure that I am studying program behavior rather than some
implementation details, I plan to focus on the results of running
programs. I approach the second predicate by modeling the
``consensus'' of modern programming language with SMoL. Because SMoL
represents the semantic core, misconceptions about SMoL presumably
also apply to most modern PLs. I approach the third and last predicate
by collecting data in population that very likely have prior
experience. For example, my priliminary results are build upon data
collected in a upper-year programming language course.

In order to find a reasonably comprehensive collection of
misconceptions, I seek for inputs from students. Most prior works
...
affected by experts' blind spots. I plan to overcome experts'
blind spots by avoid presuming misconceptions. Concretely, my first
step was a two-year process to find (a) student-written programs that
tend to trip up students and (b) incorrect responses to those programs
(\Cref{s:gen-prob-quizius}), using a tool for the purpose
(\Cref{s:quizius}).

Another source of misconceptions are student responses in the tutoring
system. The tutoring system, which I will explain in details when I
talk about RQ2 (\Cref{s:methodology-rq2}), includes many
multiple-choice questions (MCQs) that asks students to predict the
result(s) of running a program. MCQ is a commonly used task format for
detecting misconceptions [CITE]. MCQs in their own only detect
\emph{patterns of errors}. To detect \emph{misconceptions} -- student
conceptions that produces pattern of errors, I need student-written
explanations for their wrong answers. To this end, I plan to ask a
follow up questions after the MCQs: when students make a wrong choice,
the follow-up asks \emph{why} they made the choice.

I expect to find multiple misconceptions. Each misconception is
represented by a collection of program-result-explanation triples.
There should be a small number of programs. For each of these
programs, a wrong result is commonly predicted by students from
various population. Furthermore, these students give similar explanations
for all the wrong answers. I also would like to provide a definitional
interpreter for the misconception.

For example, I might find a misconception called \textbf{DefByRef}:
when students are asked to predict the result of
\begin{lstlisting}
  (defvar x 12)
  (defvar y x)
  (set! y 0)
  ...
  y
\end{lstlisting}
many of them responded \lstinline|12 0|; and when asked to predict
\begin{lstlisting}
  (defvar x 12)
  (defvar y x)
  (set! x 0)
  ...
  y
\end{lstlisting}
many of them responded \lstinline|0 12|; and these students all
explained something like ``\lstinline|y| is \lstinline|x|''. To
represent this misconception, I provide a interpreter that almost
works like the referential interperter of SMoL, but \lstinline|defvar|
aliases variables.

\section{Plan for \textbf{RQ2}}%
\label{s:methodology-rq2}
...
I have built a tutor that incorporate these ideas
(\Cref{s:system-tutor}).

This tutor uses a lot of MCQs illustrated in
\Cref{f:smol-tutor-example}.
