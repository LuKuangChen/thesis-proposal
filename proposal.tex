...
machines, and language levels are helpful. I have created a tutor
(\Cref{s:system-tutor}) that incorporates all these ideas
(\Cref{s:tutor-uiux}) and have collected data from all populations. I
plan to explore more literature and to improve the tutor if I find new
helpful techniques.
...
Each tutorial might be delivered as multiple sections to avoid forcing
...
\begin{itemize}
  ...
  system because we do not mind students doing badly on earlier tasks
...
\begin{quote}
  When we see a variable reference, how do we find the corresponding
  declaration, if any?
\end{quote}
...
In Quizius, experts create a prompt; in our case, students were to
...
imagine. In our case, I seeded Quizius with programs from prior papers
(\Cref{s:background-misconceptions}), which gives the first few
students programs to respond to. However, I found that Quizius
significantly expanded the scope of our problems and misconceptions.
In our final instrument, most programs were directly or indirectly
...
\begin{enumerate}
  ...
  \item I felt the existing programs were insufficient to cover the
        ideas we wanted students to work through. For instance, SMoL
...
answers from the SMoL Quizzes (and constructing our best guess of
...
(\Cref{s:smol-quizzes}), we provided students with refutation texts
