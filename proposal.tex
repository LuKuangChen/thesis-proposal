...
\newcommand{\rqII}{How effective is a tutoring system driven by cognitive and educational principles at correcting these misconceptions?}
...
\title{Identifying and Correcting Programming Language Behavior Misconceptions}
...
plans for the RQs (\Cref{s:plan-rq1,s:plan-rq2}). After that, I describe the preliminary results (\Cref{s:preliminary-results} and propose my next steps (\Cref{s:future-work}).
...
\begin{description}
  \item[University 1] includes students in a ``Principles of Programming
  ...
  \item[University 2] includes students at a primarily public university in the US. It is
  ...
  \item[Textbook] includes visitors to the website of a
...
multiple-choice questions (MCQs) that ask students to interpret
...
each cluster, I obtained a set of rationales.

After running Quizius in the course (\Cref{s:populations}),
I took over as an expert. Determining which is the right
answer is easy, whereas expert knowledge is useful in
...
\end{lstlisting}
  ...
  \item I deferred the local binding forms to the end, since I felt
        they were less important. I therefore rewrote programs that use
...
These works often mention \emph{notional machines}
...
This section describes the current status of the Tutor.
...
spent about 9.8 minutes (median).
...
original university and University 2, I obtained a \pValue~=~2.013e-07.
Between the original university and the online population, I obtained a
...
Of course, other threats to generalizability remain: both University
1 and University 2 use the same textbook; SMoL's Lispy syntax might
have biases in probing errors. I plan to address these issues in the
future (\Cref{s:future-work}).
...
So I plan to change the question format into a more consistent style. So I plan to replace the
\textbf{scope} prompt with
\begin{quote}
  How do variable referencess (e.g., the \lstinline|x| in \lstinline|(+ x 1)|) , \lstinline|defvar|s, and \lstinline|deffun|s interact with each other?
\end{quote}
I plan to use a similar prompt in later tutorials. For example, in \textbf{mut-vars},
I would want the Tutor to ask
\begin{quote}
  How do \lstinline|set!|s interact with other language constructs (namely, \lstinline|defvar|s, \lstinline|deffun|s, and variable references)?
\end{quote}
