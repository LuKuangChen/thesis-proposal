...
recognition of this deep commonality, in this paper I choose to call
...
%         find weaknesses in my manual analysis and hold potential for
...
SMoL defines a semantic but not the syntax. However, to repsent SMoL
...
languages. I present machine-translated Python and JavaScript versions
of my programs in \supplRef{Translated\_Programs.html}. Furthermore, I
intend to make a multi-lingual tutor in the future
(\Cref{s:future-work}).
...
them have expressed interests in using my instruments. So the final
...
trip up students and (b) incorrect responses to those programs
...
to answer my RQ2, which I explain in details in \Cref{s:plan-rq2}. The
tutor includes many tasks that asks students to predict the result(s)
...
have been modified to fit tutoring purpose. Perhaps because of the
...
I plan to change the tutor such that it asks students' to explain
their prediction. I plan to eyeball the explanations to find new
misconceptions. This piece of data also helps me to confirm that
anticipated wrong answers do represent the expected misconceptions.

\subsection{Reduce Syntactic Bias}%
\label{s:reduce-syntactic-bias}
...
current results find that some students think the following program
...
from my instruments, there is still a risk of having biase in student
errors. I plan to adjust the process descrbed in
\Cref{s:find-misconceptions-with-tutor} such that it presents programs
in multiple languages: the SMoL version, a JavaScript translation, and
a Python version.
...
incorporates all these ideas (\Cref{s:tutor}) and have collected
data from all populations. I plan to explore more literature and to
improve the tutor if I find new helpful techniques.
...
Each tutorial might be delivered as multiple modules to avoid forcing
students to continuesly work for more than 30 minutes.
...
the strategy of assigning panelty is non-trivial:
\begin{itemize}
  ...
  \item A alternative common strategy is to grade by completion.
...
The tutor provides opportunities for making case comparison: each
...
SMoL. To incorporate the idea of case comparsion, the tutor currently
prompts students to compare these cases and then to summarize the
rules of program behavior at the end of \emph{some} tutorials. I plan
to expand this to \emph{all} tutorials and use a consistent prompt.
The prompt should asks about the behavior of new language constructs
and their interaction with all previously introduced constructs.
...
\begin{itemize}
  ...
  \item Students tend to give brief response possibly because the
  questions is asking for a lot and because students are tired when
...
\begin{quote}
  ...
  \end{quote}
  Otherwise, please \emph{response with} what you are uncertain about these language constructs.
\end{quote}
...
\begin{quote}
  ...
  Otherwise, please \emph{response with} what you are uncertain about these language constructs.
\end{quote}
I am hoping these new prompt are

\subsection{Notional Machine}%
\label{s:notional-machine}

Prior research argues (\Cref{s:pedagogic-techniques})that illustrating
how programs runs with notional machines (NMs) is benefitial for
students. I have created a web-based notional machine
\begin{center}
  \href{https://lukuangchen.github.io/stacker-2023}{https://lukuangchen.github.io/stacker-2023}
\end{center}

The notional machine can be configurated with URL parameters. For
example,
...
link} opens the NM with a program embeded.
...
To fully answer my research questions, I should also argue that my
tutor is effective at correcting misconceptions. In this secton, I
...
analysis do I want to perform on the data.
...
tutor seeems to be correcting misconceptions. If studnets perform
worse in later taks, the tutor is potentially doing harm.
...
explanations and other texts in the tutor and resolve those
dependencies if any.

\section{Generating and Collating Problems}%
\label{s:gen-prob-quizius}

% Additionally, in this paper, I intentionally use the word
...
%   \item I can get past the expert blind spot,

%   \item I have a sense of what misconceptions students have, and

%   \item I can generally associate wrong answers with specific
%         misconceptions, approaching a concept inventory.

% \end{enumerate}

This section describes preliminary results on finding misconceptions
from student-written programs (\Cref{s:quizius}) and what I have done
to distill misconception-detecting programs from student inputs
(\Cref{s:smol-quizzes,s:tutor-refine-program-set}).

\subsection{Generating Problems Using Quizius}
\label{s:quizius}

My main solution to the expert blind spot is to use the Quizius system
...
The Quizius instrucment was set up by other people. I analyzed the
collected data.
...
Having curated the output, I had to confirm that these programs were
...
sense: namely, a brief test of knowledge) that I call the SMoL
...
Each quiz is a sequence of MCQs. The options are from the Quizius
data. In addition, there is an ``Other'' option. If chosen, the quiz
gave the user a text box with the caption ``Please specify''. The goal
here was to record any other answers, which in turn might lead to
fresh misconceptions.

The quizzes were administered in the third year in University 1.
...
beginning. Similarly, I wanted programs that are syntactically
...
Students only received feedback after having completed a whole quiz.
At the end of each quiz, they received both summative feedback
...
programs, but I have little reason to believe that they did (and
certainly they asked few questions on the class forum about them).

Due to space limitations, I present the entire instrument in
\supplRef{SMoL Quizzes/instrument}. Here I focus on a few programs
where student choices correspond to misconceptions identified earlier,
thereby also showing that the curated programs are still effective. I
use a * to indicate the correct answer (which also matches the
implementation's output).

% Syntactically, \lstinline|#t| and \lstinline|#f| are true and false,
% while \lstinline|#(...)| is a vector.
...
\begin{table}[t]
  ...
  \caption{Some questions and student answers from \quizFun. Each
    ...
    with \texttt{*}. Wrong answers that I discuss are
    marked with $^\dag$.}
  \label{t:quizzes-data-fun}
\end{table}
\Cref{t:quizzes-data-fun} lists programs in \quizFun{} that I
...
\end{table}
\Cref{t:quizzes-data-hof} lists interesting programs after the
addition of closures and higher-order functions. It extends what we
saw with loops in \Cref{s:intro}: that students have misconceptions
about their interaction with mutable variables:
\begin{itemize}
...
this paper. I bootstrap it from the SMoL Quizzes, but made the
following alterations:
\begin{enumerate}

  % fill gaps in goal sentences
  \item I felt the existing programs were insufficient to cover the
        ideas I wanted students to work through. For instance, SMoL
        Quizzes did not alias vectors using \lstinline|defvar|, so we
        added this program:
...
\end{lstlisting}
  ...
  \item I removed programs of a ``Lispy'' nature (See
        \Cref{s:reduce-syntactic-bias}).
...
introduce programs. Starting with the 37 programs in the SMoL Quizzes,
I added 52 more programs to arrive at a total of 89.

In addition to generating this set of programs, I also modified the
answer options. In addition to retaining the correct and incorrect
answers from the SMoL Quizzes (and constructing my best guess of
analogous incorrect answers for the new problems), I added more wrong
answers to for the reasons given in \Cref{s:interpreting-tasks}. In
general, I ended up increasing the number of choices substantially:
only 8 out of the 89 questions have three or four choices; 70
questions have 5--8 choices; and 11 questions have 9--14 choices.

\section{What Pedagogic Techniques are Effective?}%
\label{s:pedagogic-techniques}

The SMoL Tutor is firmly grounded in one technique from cognitive and
educational psychology. The fundamental problem is: how do you fix a
misconception? One approach is to only ``present the right answer'',
for fear that discussing the wrong conception might actually reinforce
it. Instead, there is a body of literature starting from
...
The SMoL Tutor's content structure is also influenced by work on
\emph{case comparisons} (which draws analogies between examples).
...
before or not at all), are associated with better learning outcomes.
  ...
  [FILL Language Levels] Prior research argues for the benefits of
\emph{language levels} [CITE]: a langauge has better be taught as a
sequence of languages, where each later language includes more
langauge constructs than the previous one.

\section{The SMoL Tutor}%
\label{s:tutor}

% So far, I have focused on identifying problems. As noted earlier
% (\Cref{s:smol-quizzes}), I provided students with refutation texts
% after the quizzes, but it is unclear to what extent students read,
% understood, or internalized these. I also wanted to determine whether
% other populations run into these issues, and it was unclear whether we
% would be able to administer passive quizzes to them.

% Furthermore, while the quizzes may be a useful diagnostic, my goal is
...
% In response, I decided to create a \emph{tutor}: the SMoL Tutor. It is built
% around my quiz instruments and the detected misconceptions. We
% describe the Tutor from a user's perspective in \Cref{s:tutor}, and
% explain how it was populated from the
% SMoL Quizzes in \Cref{s:tutor-refine-program-set}.
% I then discuss what
% I learned from its data (\Cref{s:misconceptions-tutor}), and finally
...
descrbed in \Cref{s:interpreting-tasks}. The Tutor also asked students
to perform some other activities, like showing programs and asking for
their heap content, which I do not cover in this paper.

% Each tutorial consists of a sequence of questions. Most questions in
% the Tutor are \newterm{interpreting questions}.\footnote{The Tutor
% also asked students to perform some other activities, like showing
% programs and asking for their heap content, which I do not cover in
% this paper.} These questions (illustrated in
% \Cref{f:smol-tutor-example}) are versions of the SMoL Quizzes
% (obtained by the process described in
% \Cref{s:tutor-refine-program-set}). In each of these questions,
% students are shown a program and asked to predict the program's
% running result(s) by answering an MCQ (with an ``Other'' option).
% After making a choice, students receive feedback. If a student chooses
% incorrectly, they are (a) given an explanation that is \emph{based on
% the misconception associated with that wrong answer} (or a generic
% one, if there is not a specific misconception), and then (b) asked to
...
%         This is
%         intentional. First, I wanted to force reflection on the
%         explanation just given, whereas with an
%         MCQ, students could just guess. Second, I felt that students would
%         find typing more onerous than clicking. In case students
%         had just guessed on a question, my hope is that the penalty of having to
...
% I exclude this module from my analysis and instruments, and indeed
...
I now examine what I learned from the interpreting
questions in terms of program behavior
misconceptions. But first, I explain how I associate incorrect
answers with misconceptions.

I do not present the results of and \textbf{local}
(\Cref{t:tutor-goal-sentences}), because they focus on constructs not
usually found in non-Lispy languages. They were included in the Tutor
to help students write programs for the course, but are not
interesting from a SMoL perspective.

\subsection{Misinterpreters}
\label{s:misinterp-intro}

In principle, an expert can identify what misconceptions a particular
incorrect answer might correspond to. In practice, I found this
rather difficult for three reasons. First, with 89 programs, each with
several answers, it's easy to make mistakes. Second, my own expert blind
spot may prevent me from seeing an interpretation that would
lead to an additional association. Finally, and specific to my case,
since I had either curated or written all the programs and answers,
I was likely to miss some associations I had not intended.

To address this problem, I formalized misconceptions as
interpreters. That is, for each misconception I created a
...
By running programs through the misinterpreters, I can more uniformly
and rigorously identify \emph{all} the misconceptions associated with
a wrong answer. Furthermore, if I identify a new misconception (or
alter a misinterpreter), it is easy to automatically re-classify all
the answers. By using misinterpreters, I indeed found new
...
identify with a $^\ddag$), which need to be validated in the future.\footnote{To keep
  ...
  ask students for rationales for their programs. However, I could
...
(b) difficult for
us to attribute to a misconception, or (c) appeared to me to be
...
misconception.\footnote{An astute reader may well imagine another
  misinterpretation; but I presumably did not find evidence of it in our
...
% The misinterpreters helped me find misinterpretations that I had
...
% However, my misinterpreters made me realize it can also be explained
% by \miscon{FlatEnv}. This could also explain why I see a difference in
% error rate when the formal and actual parameter have the same
% name (as mentioned in \Cref{s:misconceptions-quiz:state}).

% Tutor's refined program sets and misinterpreters also helped me to
...
% To summarize, I used the SMoL Tutor, with an expanded set of
% programs, to investigate student misconceptions. I implemented the
% idea of misinterpreters to help me properly classify student
...
misinterpreters, I found far fewer problems than I would have liked.
...
Fortunately, I were able to deploy the Tutor on two other
...
To evaluate, I computed a Spearman's rank correlation $\rho$, ranking
...
the online population, I obtain a \pValue~<~2.2e-16. These show that
