...
I plan to change the tutor such that it collects data to uncover these
unknown misconceptions and to confirm known ones.
...
interpreters \emph{misinterpreters}.
...
penalty. Designing the strategy of assigning penalty is non-trivial:
\begin{itemize}
...
explicitly require them to run programs in a NM visualization tool.
...
Quizzes.
% There were three quizzes, ordered by linguistic
...
\section{Preliminary List of Misconceptions}%
...
perspective of the data in this paper). I started with misinterpreters
representing the misconceptions described in
...
\section{How Effective Is the Tutor?}
...
\section{How Generalizable is My Observations?}
...
help students to run the program in DrRacket, where a SMoL
implementation is provided.

This approach is inconvenient both for the students and for me. For
students, they have to click, pastes and run, which is less convenient
than a single click. For me, because students run program in DrRacket
rather than ``within'' the tutor, I do not know whether they have run
the programs. This limit my ability to claim that the tutor is using
NM to help teaching.
...
have not analyzed the data, which I plan to do in Spring 2024.

I have not changed the logging systems to log when students click the
buttons. I plan to make the change before Spring 2024.

Another issue with the current design is that students might run a
program before giving an answer. I don't know yet how often this
happens. But given that students receive penalty (\Cref{s:penalty})
...
from my instruments, there is still a risk of having biases in student
...
\subsection{Use Case Comparisons More Consistently}
...
\subsection{Options in Interpreting Tasks}

The tutor is adding extra options in interpreting tasks in an ad hoc
...
\subsection{Other Changes in the Tutor}

By the end of Fall 2023, I also want to improve the SMoL Tutor at
least on the following aspects:
\begin{enumerate}
  \item The tutor currently does not ask students to explain when they
        gave a wrong answer in an interpreting question. This limit my
        ...
        % misconceptions.
  \item As I said in \Cref{s:effect-tutor}, for many of the
        ...
        questions such that every misconception has at least two
        questions that aim at correcting the misconception.
  \item The tutor is not randomizing the order of questions. This make
  ...
  \item After randomizing question orders, I would need to check
        carefully the dependencies between explanations and to
        resolve these dependencies, if any.
  \item The tutor is using many variable names, some are separated by
        dash (e.g., \lstinline|get-x|). Students might misread these
        ...
        \lstinline|x|). These misreading add noise to the data. So I plan
        to use consistent and simple variable names in all programs.
\end{enumerate}

\subsection{Explore More Pedagogic Techniques}
...
country dring Spring 2024, so I do not plan to make more progress
