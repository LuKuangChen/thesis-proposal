...
(\Cref{s:generality-tutor}). For example, my priliminary results are
build upon data collected in a upper-year programming language course
at Brown. [FILL list the data sources]
...
\Cref{s:misconceptions-tutor} list misconceptions found with this
process.
...
students to predict the result(s) of running a program. I have found
several unanticipated patterns of errors
(\Cref{s:misconceptions-tutor}), which suggest the existence of
unknown misconceptions. The current system do not collect students'
rationale for their answers. So I do not know what are the underlying
misconceptions (although some of the wrong answers are somewhat
self-explanatory). I plan to change the system such that it asks
students' for their explanations when they give a wrong answer. I plan
to eyeball the explanations to find new misconceptions.
...
plan to do the same things for newly found/confirmed misconceptions.
...
pedagogic techniques are helpful (\Cref{s:background}). My current
takeaway is that refutation text, case comparison, and notional
machines are helpful. And I have created a tutoring system (hereafter,
the tutor) (\Cref{s:system-tutor}) that incorporates refutation text
and case comparison (\Cref{s:tutor-uiux}). I am adding notional
machine to the tutor and plan to explore more literature and to
improve the tutor if I find new helpful techniques.

I would like to first explain key aspects of the tutor
(\Cref{s:tutorials,s:interpreting-task}) and then how the tutor
incorporates ideas of refutation text (\Cref{s:refutationt-text}),
case comparsion (\Cref{s:case-comparison}), and notional machines
...
Inspired by prior research on language levels (\Cref{s:background}),
my sequence of tutorials is ordered, and each later tutorials cover
one or more language constructs (\Cref{t:tutorials}).
...
reconsider the program in \Cref{s:interpreting-task}. If a student
...
\begin{itemize}
  \item How effective is the tutor at stopping students from holding
        misconceptions?
  \item How close are students' final conception to the true
        conception?
\end{itemize}
...
\begin{itemize}
  ...
  \item In each tutorial, a misconception is either not represented by
        any interpreting tasks or represented by exactly two tasks.
  \item Within each tutorial, the order of interpreting tasks is
        randomized.
\end{itemize}

For each tutorail and each misconception appear in the tutorial, I
compare student performance on the \emph{first} question that involve
the misconceptions with performance on the \emph{second} question. If
the difference is clear (i.e., ``statistically significant''), I
deduce that the tutor is effective at correcting the misconception.
