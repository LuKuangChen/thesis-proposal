...
\newcommand{\rqI}{What are the program-behavior misconceptions that apply to most modern programming languages and present (even) in students with prior programming backgrounds?}
\newcommand{\rqII}{What would be a tutoring system that aims at correcting the misconceptions, draws on cognitive and educational psychology concepts, utilizes a notional machine, and seems to be effective?}
...
\begin{abstract}
  ...
  In this proposal, I present a curated list of misconceptions, and an
  instrument to detect them. These are distilled from student work
  over several years and match and extend prior research.  I also
  ...
  I have tested the tutor in multiple settings.
  Our data consistently show that (a) the misconceptions I tackle are
...
them. I draw on this literature. In particular, it is common in the
literature to talk about a ``two-loop'' architecture \cite{vanlehnBehaviorTutoringSystems2006a} where the outer
loop iterates through ``tasks'' (i.e., educational activities) and the inner loop iterates through UI events within a task. I follow the same
...
and correcting them. I are not aware of a tutoring system (in computer science) that has
...
\section{Background: Program Behavior}
...
To answer this RQ, I need to first define what I mean by ``apply to
most modern programming languages''. As I described in the
Introduction (\Cref{s:intro}), most modern languages share a semantic
core. I have modelled this core as SMoL (\Cref{s:smol}). Because SMoL
represents the semantic core, misconceptions about SMoL presumably
also apply to most modern PLs.

To find misconceptions that ``present (even) in students with
prior programming backgrounds?'', I have been collecting data from
multiple populations that very likely have prior experience. For
example, my priliminary results are build upon data collected in a
upper-year programming language course at Brown.
...
misconceptions with student inputs. This approach can't possibly find
misconceptions not considered by experts and hence is necessarily
affected by experts' blind spots. To overcome experts' blind spots, I
analyze a dataset produced by a two-year process to find (a)
student-written programs that tend to trip up students and (b)
incorrect responses to those programs (\Cref{s:gen-prob-quizius}),
using a tool for the purpose (\Cref{s:quizius}). I have been cleaning
up those programs (\Cref{s:smol-quizzes,s:tutor-refine-program-set})
and confirming that these programs are still tricky in the same way
(\Cref{s:misconceptions-quiz,s:misconceptions-tutor}).

I have been creating a tutoring system to answer my RQ2. The tutoring
system collects more student data, which becomes another source of
misconceptions. The tutoring system, which I explain in details when I
talk about RQ2 (\Cref{s:plan-rq2}), includes many tasks that asks
students to predict the result(s) of running a program. I plan to
change the tutor such that it asks students' for their explanations
when they give a wrong answer. I plan to eyeball the explanations to
see if there are any new misconceptions.
...
plan to do the same things for potentially newly found/confirmed
misconceptions.

Eventually, I expect to build a table of misconceptions. Each
misconception comes with two or more programs. For each of these
...
Furthermore, these students give similar explanations for all the
wrong answers. For example, I might find a misconception called
...
Preliminary results can be found in \Cref{s:misconceptions-tutor}.
...
Prior works suggest that presenting refutation text and encouraging
students to compare cases are helpful for learning, and that notional
machines are useful for explaning program behavior.

I would first explain the major task format
(\Cref{s:interpreting-task}) in the tutor and then explain how I would
let the tutor incorporate ideas of refutation text
(\Cref{s:refutationt-text}), case comparsion
(\Cref{s:case-comparison}), and notional machines
(\Cref{s:notional-machine}). Finally, I explain how I plan to evaluate
the effectiveness of the tutor (\Cref{s:evaluation}).

\subsection{Interpreting Task}%
\label{s:interpreting-task}

To build a tutor that incorporate these ideas, I plan to includes lots
of multiple-choice questions (MCQs) in the tutor. I call these
interpreting tasks because they ask students to interpret programs:
each MCQ presents a program, for example,
\begin{lstlisting}
  (defvar x 12)
  (defvar y x)
  (set! y 0)
  ...
  y
\end{lstlisting}
and then asks students to choose the results of the program.

\paragraph{The MCQ Options} The options include the correct answers and
wrong answers that correspond to known misconceptions (in this case,
only \lstinline|0 0|, which correspond to \miscon{DefByRef}), among
other options that I will explain shortly. Because students might give
wrong answers that I do not anticipate, I plan to include an ``Other''
option that, once choosen, allows students to enter arbitrary answer.
If an MCQs only have a small number of options, students have a great
chance (in this case, 50\%) of just guessing the right answer or
successfully using a process of elimination. So I plan to add
additional options that are not obviously wrong: options generated by
moving and/or copying numbers in the current options. In this case, I
will add \lstinline|12 12| and \lstinline|0 12|. In summary, each MCQ
...
making a random choice, the tutor should gives panelty. Deciding the
panelty is a non-trivial question. A common strategy is to grade
students by their correctness. However, grading by correctness is not
suitable for a tutoring system because we do not mind students doing
badly on earlier tasks as long as they eventually master the concepts.
Actually, if students generally do perfectly on all tasks, that is a
failure of the tutoring system -- it is likely not teaching anything
to the students. If grading by correctness is not a suitable option, a
common alternative is to grade by completion. However, grading by
completion alone encourage students to rush through the tutor and
hence might encourage students to make random choices. In short, I
would like to make students feel as follows:
\begin{itemize}
  \item They won't lose grade as long as they finish the tutor.
  \item They should avoid giving wrong answers.
\end{itemize}
To encourage this feeling, I do the following: when I deployed the
tutor in my institution, I graded by completion\footnote{However, I
have no control over how other institutions use the tutor.}; when a
student fails a task, the tutor gives an \emph{extra} task:
\begin{enumerate}
...
\subsection{Design the Tutor with Refutation Text and Other Explanations}%
\label{s:refutationt-text}

The tutor presents a refutation text when a student seems to hold a
misconception. Let's reconsider the program in
\Cref{s:interpreting-task}. If a student answers \lstinline|0 0|,
which suggests they hold the \miscon{DefByRef} misconception, the
tutor responds
\begin{quote}
  The correct answer is \lstinline|12 0|. \lstinline|y| was bound to
  \lstinline|12| (i.e., the value of \lstinline|x|) rather than to
  \lstinline|x|. So changing the value of \lstinline|y| does not
  changes the value of \lstinline|x|.
\end{quote}
...
tutor responds
\begin{quote}
  The correct answer is \lstinline|12 0|. The first definition binds
  \lstinline|x| to \lstinline|12|. The second definition binds
  \lstinline|y| to the value of \lstinline|x|, which is
  \lstinline|12|. The \lstinline|set!| mutates the binding of
  \lstinline|y|, so \lstinline|y| is now bound to \lstinline|0|.
\end{quote}

\subsection{Design the Tutor with Case Comparison}%
...
tutor prompts students to compare these cases and then to summarize
the rules of program behavior.

It is impractical to ask students to compare tens or even hundreads of
programs. So
...
\section{Proposal Roadmap}
\label{s:roadmap}
\label{s:univ-1-course}

This proposal describes a four-year effort to obtain a quality instrument
...
I then present the tutor (\Cref{s:system-tutor}). In particular, we
present the list of misconceptions---grounded in student data---that
are the focus of this project (\Cref{s:misconceptions-tutor}). I also
...
while \verb|#(...)| is a vector. I use a * to indicate the correct
